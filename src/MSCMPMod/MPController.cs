...
namespace MSCMP {
	...
	class MPController : MonoBehaviour {
		...
		void OnLevelSwitch(string newLevelName) {
			if (currentLevelName == "GAME") { gameWorld.OnUnload(); }
			...
			if (currentLevelName == "GAME" && newLevelName == "MainMenu") {
				if (netManager.IsOnline) { netManager.Disconnect(); }
			}
		}
		...
		void OnGUI() {
			...
			if (ShouldSeeInvitePanel()) { UpdateInvitePanel(); }

#if !PUBLIC_RELEASE
			DevTools.OnGUI();

			if (DevTools.netStats) { netManager.DrawDebugGUI(); }
		...
		void UpdateFriendList() {
			...
			if (timeToUpdateFriendList > 0.0f) { return; }
		...
		private void UpdateInvitePanel() {
			...
			Rect invitePanelRect = new Rect(Screen.width - invitePanelWidth - 10.0f,
					Screen.height / 2 - invitePanelHeight / 2, invitePanelWidth, 20.0f);
			...
			if (onlineFriendsCount == 0) {
				...
				bool playerIsOffline = Steamworks.SteamFriends.GetPersonaState() ==
						Steamworks.EPersonaState.k_EPersonaStateOffline;
				if (playerIsOffline) {
			...
			friendsScrollViewPos =
					GUI.BeginScrollView(invitePanelRect, friendsScrollViewPos,
							new Rect(0, 0, invitePanelWidth - 20.0f, 20.0f * onlineFriendsCount));
			...
			if (lastIndex > onlineFriendsCount) { lastIndex = onlineFriendsCount; }
			for (int i = firstVisibleFriendId; i < lastIndex; ++i) {
				...
				if (inviteCooldown > 0.0f) { continue; }
				...
				if (GUI.Button(friendRect, "Invite")) {
					if (netManager.InviteToMyLobby(friendSteamId)) {
						...
						UI.MPGUI.Instance.ShowMessageBox(
								"Failed to invite friend due to steam error.");
					}
				}
			}
		...
		/// </summary>
		void FixedUpdate() {
			Utils.CallSafe("FixedUpdate", () => { netManager.FixedUpdate(); });
		}
		...
		void LateUpdate() {
			Utils.CallSafe("Update", () => {
				...
				if (IsInvitePanelVisible()) { UpdateFriendList(); }
		...
		/// <summary>
		/// Wrapper around unitys load level method to call OnLevelSwitch even if level
		/// is the same.
		/// </summary>
		/// <param name="levelName">The name of the level to load.</param>
		public void LoadLevel(string levelName) { Application.LoadLevel(levelName); }
