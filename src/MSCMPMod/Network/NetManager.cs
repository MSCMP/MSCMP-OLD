...
namespace MSCMP.Network {
	class NetManager {
		...
		private Steamworks.Callback<Steamworks.GameLobbyJoinRequested_t>
				gameLobbyJoinRequestedCallback = null;
		private Steamworks
				.Callback<Steamworks.P2PSessionRequest_t> p2pSessionRequestCallback = null;
		private Steamworks
				.Callback<Steamworks.P2PSessionConnectFail_t> p2pConnectFailCallback = null;
		private Steamworks.CallResult<Steamworks.LobbyCreated_t> lobbyCreatedCallResult =
				null;
		private Steamworks.CallResult<Steamworks.LobbyEnter_t> lobbyEnterCallResult =
				null;
		public enum Mode { None, Host, Player }

		public enum State { Idle, CreatingLobby, LoadingGameWorld, Playing }
		...
		const float TIMEOUT_TIME =
				60.0f; // This was raised as too many people had issues with joining onn slow
							 // PCs. A better solution will be implemented in the future.
		...
		void OnP2PConnectFail(Steamworks.P2PSessionConnectFail_t result) {
			Logger.Error(
					$"P2P Connection failed, session error: {Utils.P2PSessionErrorToString((Steamworks.EP2PSessionError)result.m_eP2PSessionError)}, remote: {result.m_steamIDRemote}");
		}
		...
		void OnP2PSessionRequest(Steamworks.P2PSessionRequest_t result) {
			if (Steamworks.SteamNetworking.AcceptP2PSessionWithUser(
							result.m_steamIDRemote)) {
		...
		void OnLobbyCreated(Steamworks.LobbyCreated_t result, bool ioFailure) {
			if (result.m_eResult != Steamworks.EResult.k_EResultOK || ioFailure) {
				Logger.Log(
						$"Failed to create lobby. (result: {result.m_eResult}, io failure: {ioFailure})");

				MPGUI.Instance.ShowMessageBox(
						$"Failed to create lobby due to steam error.\n{result.m_eResult}/{ioFailure}",
			...
			// Setup local player.
			players[0] =
					new NetLocalPlayer(this, netWorld, Steamworks.SteamUser.GetSteamID());
		...
		void OnLobbyEnter(Steamworks.LobbyEnter_t result, bool ioFailure) {
			if (ioFailure ||
					result.m_EChatRoomEnterResponse !=
							(uint)Steamworks.EChatRoomEnterResponse
									.k_EChatRoomEnterResponseSuccess) {
				Logger.Error(
						"Failed to join lobby. (reponse: {result.m_EChatRoomEnterResponse}, ioFailure: {ioFailure})");
				MPGUI.Instance.ShowMessageBox(
		...
		void RegisterProtocolMessagesHandlers() {
			netMessageHandler.BindMessageHandler(
					(Steamworks.CSteamID sender, Messages.HandshakeMessage msg) => {
						HandleHandshake(sender, msg);
					});

			netMessageHandler.BindMessageHandler(
					(Steamworks.CSteamID sender, Messages.HeartbeatMessage msg) => {
			...
			netMessageHandler.BindMessageHandler(
					(Steamworks.CSteamID sender, Messages.HeartbeatResponseMessage msg) => {
			...
			netMessageHandler.BindMessageHandler(
					(Steamworks.CSteamID sender, Messages.DisconnectMessage msg) => {
		...
		/// <summary>
		/// Get network clock with the milliseconds resolution. (time since network
		/// manager was created)
		/// </summary>
		/// <returns>Network clock time in miliseconds.</returns>
		public ulong GetNetworkClock() {
			return (ulong)(
					(DateTime.UtcNow - this.netManagerCreationTime).TotalMilliseconds);
		}
		...
		/// <returns></returns>
		public bool BroadcastMessage<T>(
				T message, Steamworks.EP2PSend sendType, int channel = 0)
				where T : INetMessage {
			if (players[1] == null) { return false; }

			MemoryStream stream = new MemoryStream();
			if (!WriteMessage(message, stream)) { return false; }

			foreach (NetPlayer player in players) {
				if (player is NetLocalPlayer) { continue; }
		...
		public bool SendMessage<T>(
				NetPlayer player, T message, Steamworks.EP2PSend sendType, int channel = 0)
				where T : INetMessage {
		...
		private void OnGameLobbyJoinRequested(
				Steamworks.GameLobbyJoinRequested_t request) {
			Steamworks.SteamAPICall_t apiCall =
					Steamworks.SteamMatchmaking.JoinLobby(request.m_steamIDLobby);
			if (apiCall == Steamworks.SteamAPICall_t.Invalid) {
				Logger.Error(
						$"Unable to join lobby {request.m_steamIDLobby}. JoinLobby call failed.");
				MPGUI.Instance.ShowMessageBox(
		...
		/// </summary>
		/// <returns>true if lobby setup request was properly sent, false
		/// otherwise</returns>
		public bool SetupLobby() {
			Logger.Log("Setting up lobby.");
			Steamworks.SteamAPICall_t apiCall = Steamworks.SteamMatchmaking.CreateLobby(
					Steamworks.ELobbyType.k_ELobbyTypeFriendsOnly, MAX_PLAYERS);
			if (apiCall == Steamworks.SteamAPICall_t.Invalid) {
		...
		/// </summary>
		/// <returns>true if there is another player connected and playing in the
		/// session, false otherwise</returns>
		public bool IsNetworkPlayerConnected() { return players[1] != null; }
		...
		public void Disconnect() {
			BroadcastMessage(
					new Messages.DisconnectMessage(), Steamworks.EP2PSend.k_EP2PSendReliable);
			LeaveLobby();
		}
		...
		private void HandleDisconnect(bool timeout) {
			...
			if (IsHost) {
				...
				MessagesList.AddMessage(
						$"Player {players[1].GetName()} disconnected. ({reason})",
		...
		private void UpdateHeartbeat() {
			if (!IsNetworkPlayerConnected()) { return; }
		...
		/// </summary>
		public void FixedUpdate() { netWorld.FixedUpdate(); }
		...
		public void Update() {
			...
			foreach (NetPlayer player in players) { player?.Update(); }
		}
		...
		public void DrawNameTags() {
			if (players[1] != null) { players[1].DrawNametag(); }
		}
		...
		private void HandleHandshake(
				Steamworks.CSteamID senderSteamId, Messages.HandshakeMessage msg) {
			if (IsHost) {
				if (players[1] != null) {
				...
				// Player can be spawned here safely. Host is already in game and all game
				// objects are here.
				...
				MessagesList.AddMessage(
						$"Player {players[1].GetName()} joined.", MessageSeverity.Info);
			} else {
				if (players[1] == null) {
				...
				// Host will be spawned when game will be loaded and OnGameWorldLoad callback
				// will be called.
			}
		...
		public void OnGameWorldLoad() {
			...
			if (IsPlayer) { netWorld.AskForFullWorldSync(); }
		}
		...
		public NetLocalPlayer GetLocalPlayer() { return (NetLocalPlayer)players[0]; }

		/// <summary>
		/// Get network player object by steam id.
		/// </summary>
		/// <param name="steamId">The steam id used to find player for.</param>
		/// <returns>Network player object or null if there is not player matching given
		/// steam id.</returns>
		...
		/// </summary>
		public void OnNetworkWorldLoaded() { state = State.Playing; }
		...
		public bool GetP2PSessionState(out Steamworks.P2PSessionState_t sessionState) {
			...
			return Steamworks.SteamNetworking.GetP2PSessionState(
					players[1].SteamId, out sessionState);
		}
	}
}
