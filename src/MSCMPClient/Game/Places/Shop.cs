...
/// <summary>
/// Syncs items on shelves at the store as well as the cash registers, and switches
/// behind counter.
/// </summary>
	...
	class Shop {
		...
		public Shop(GameObject shop) {
			...
			foreach (Transform transform in shopGO.GetComponentsInChildren<Transform>()) {
				...
				// Products on shelves.
				case "ActivateStore": shopProducts = transform.gameObject; break;

				// Products in pub.
				case "ActivateBar": pubProducts = transform.gameObject; break;
		...
		void HookEvents() {
			// Products on shelves.
			foreach (PlayMakerFSM fsm in shopProducts
			...
			// Products in pub.
			foreach (PlayMakerFSM fsm in pubProducts
									 .GetComponentsInChildren<PlayMakerFSM>()) {
				if (fsm.FsmName == "Buy") {
					EventHook.AddWithSync(fsm, "Check money", new Func<bool>(() => {
						if (fsm.Fsm.PreviousActiveState.Name.StartsWith("MP_") &&
			...
			// Cash register in store.
			PlayMakerFSM storeRegisterFSM =
					Utils.GetPlaymakerScriptByName(shopRegister, "Data");
