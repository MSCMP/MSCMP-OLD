...
namespace MSCMPMessages {
	class Generator {
		...
		private string GetEnumValueAsString(Type enumType, string valueName) {
			...
			return Convert.ChangeType(Enum.Parse(enumType, valueName), underlyingType)
					.ToString();
		}

		private string GetTypeName(Type type) {
			if (type.Namespace == "MSCMPMessages.Messages") { return type.Name; }
		...
		public void GenerateEnum(Type enumType) {
			...
			if (!enumType.IsEnum) { throw new Exception("The type must be enum."); }
			Type enumUnderlyingType = enumType.GetEnumUnderlyingType();

			Array values = Enum.GetValues(enumType);
			BeginBlock(
					"public enum " + enumType.Name + " : " + enumUnderlyingType.FullName);
			{
				for (int i = 0; i < values.Length; ++i) {
					string valueName = values.GetValue(i).ToString();
					WriteLine(
							valueName + " = " + GetEnumValueAsString(enumType, valueName) + ",");
				}
			}
			EndBlock();
			...
			{
				BeginBlock("public static bool IsValueValid(" + enumUnderlyingType.FullName +
						" value)");
				{
		...
		private int CountOptionals(Type messageType) {
			...
			foreach (FieldInfo field in fields) {
				...
				if (field.GetCustomAttribute<Optional>() != null) { ++optionals; }
			}
			return optionals;
		}
		...
		public void GenerateMessage(Type messageType) {
			...
			if (descriptor != null) { interfaceName = ": INetMessage"; }
			...
			if (optionals > sizeof(Byte) * 8) {
				throw new Exception(
						"Too many optionals in single message. Currently we support up to 8 optionals per message.");
			}
			...
			BeginBlock("public class " + messageType.Name + interfaceName);
			{
				...
				BeginBlock("public " + messageType.Name + "()");
				{
					foreach (FieldInfo field in fields) {
						if (field.FieldType.IsArray) {
							WriteLine(field.Name + " = new " +
									GetTypeName(field.FieldType.GetElementType()) + "[0];");
						} else if (IsNetworkMessage(field.FieldType)) {
							WriteLine(
									field.Name + " = new " + GetTypeName(field.FieldType) + "();");
						}
					}
				}
				EndBlock();
				...
				if (hasOptionals) { WriteLine("private byte optionalsMask = 0;"); }
				...
				foreach (FieldInfo field in fields) {
					...
					WriteLine(accessiblity + " " + GetTypeName(field.FieldType) + "\t" +
							field.Name + ";");
				}
					...
					BeginBlock("try");
					{
						...
						foreach (FieldInfo field in fields) {
							WriteTypeWrite(field.FieldType, field.Name,
									field.GetCustomAttribute<Optional>() != null);
						}
		...
		private void WriteTypeWrite(Type type, string name, bool isOptional) {
			...
			if (type.IsArray) {
				...
				BeginBlock(
						"foreach (" + GetTypeName(elementType) + " value in " + name + ")");
				{ WriteTypeWrite(elementType, "value", false); }
				EndBlock();
			} else if (type.IsEnum) {
				WriteLine("writer.Write((" + type.GetEnumUnderlyingType().FullName + ")" +
						name + ");");
		...
		private void WriteTypeRead(Type type, string name, bool isOptional) {
			...
			if (type.IsArray) {
				...
				Type elementType = type.GetElementType();
				WriteLine(
						name + " = new " + GetTypeName(elementType) + "[" + lenVarName + "];");
				...
				WriteLine(enumUnderlayingType.FullName + " " + valueVarName +
						" = reader.Read" + enumUnderlayingType.Name + "();");
				BeginBlock(
						"if (!" + type.Name + "Helpers.IsValueValid(" + valueVarName + "))");
				{ WriteLine("return false;"); }
				EndBlock();
		...
		private void WriteLine(string text) { writer.WriteLine(identation + text); }

		private void WriteNewLine() { writer.Write("\n"); }
		...
		private void WriteAccessors() {
			foreach (var field in fields) {
				if (field.GetCustomAttribute<Optional>() == null) { continue; }
		...
		private void WriteAccessorsForField(FieldInfo field) {
			...
			string capitalizedName =
					rawName.Substring(0, 1).ToUpper() + rawName.Substring(1);
